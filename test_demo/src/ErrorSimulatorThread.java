...
/**
 * IntHostManager for multi threads need UDPHelper.java
...
public class ErrorSimulatorThread implements Runnable {
	...
	public ErrorSimulatorThread(DatagramPacket receivePacket, InetAddress newDestinationAddress) {
			...
			this.socket = UDPHelper.newSocket();
			if (socket == null){
	...
	public void run() {
		...
		if (!simulateError(serverAddress, ErrorSimulator.DEFAULT_SERVER_PORT)) {
			...
			UDPHelper.send(socket, sendPacket);
		...
		while (true) {
			...
			receivePacket = UDPHelper.newReceive();//new packet
			UDPHelper.receive(socket, receivePacket);//receive
	...
	public boolean simulateDuplicate(InetAddress ip, int port, int value) {
		...
		for (int i=0; i<value; i++) {
			UDPHelper.print("Sending Duplicated Packet...");
			
			sendPacket = new DatagramPacket(receivePacket.getData(), receivePacket.getLength(), ip, port);
			UDPHelper.send(socket, sendPacket);
			
			UDPHelper.print("Sent, Print Duplicated Packet:");
			UDPHelper.printPacket(sendPacket);
		}
	...
	public boolean simulateIncorrectTID(InetAddress ip, int port) {
		...
		DatagramSocket new_socket = UDPHelper.newSocket();

		//System.out.print("<Error TID> Sending to port...");
		sendPacket = new DatagramPacket(receivePacket.getData(), receivePacket.getLength(), ip, port);
		//simulate a packet with the Wrong TID
		UDPHelper.send(new_socket, sendPacket);
		...
		DatagramPacket tempErrorReceivePacket = UDPHelper.newReceive();
		UDPHelper.receive(new_socket, tempErrorReceivePacket);
		
		System.out.println("Received ERORR from port: "+ tempErrorReceivePacket.getPort());
		UDPHelper.printPacket(tempErrorReceivePacket);
	...
	public boolean simulateIncorrectIP(InetAddress ip, int port) {
		...
		DatagramSocket new_socket = UDPHelper.newSocket(socket.getPort(), fakeIp);
		//System.out.print("<Error TID> Sending to port...");
		sendPacket = new DatagramPacket(receivePacket.getData(), receivePacket.getLength(), ip, port);
		//simulate a packet with the Wrong TID
		UDPHelper.send(new_socket, sendPacket);
	...
	*/
	...
	public boolean simulateIncorrectSize(InetAddress ip, int port, int errorSize) {
		...
		UDPHelper.print("Print Size Modified Packet:");
		UDPHelper.printPacket(sendPacket);
    ...
    */
	public boolean simulateCorruptedRequest(InetAddress ip, int port, int field) {
		...
		if ((second < 0) || (second != len - 1)) {
			...
			UDPHelper.print("Print receivePacket:");
			UDPHelper.printPacket(receivePacket);
			//return false;
		}
		...
		UDPHelper.print("Print Modified Packet:");
		UDPHelper.printPacket(sendPacket);
